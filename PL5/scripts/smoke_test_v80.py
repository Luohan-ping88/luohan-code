...
# ── 关键：清除所有已缓存的 src.core / core 模块 ──────────────────────────
for mod_name in list(sys.modules.keys()):
    if any(prefix in mod_name for prefix in [
        'src.core', 'core.', 'src.ai', 'src.agents', 'src.app',
        'src.config', 'src.tools', 'src.utils', 'src.service'
    ]):
        try:
...
OUT = os.path.join(_root, "logs", "smoke_v80.txt")
