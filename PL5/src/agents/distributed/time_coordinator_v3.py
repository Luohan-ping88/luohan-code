...
class TimeCoordinatorV3:
    ...
    def register_task(
        ...
        """
        actual_minutes = (
            estimated_actual_minutes
            if estimated_actual_minutes > 0
    ...
    def calculate_schedule(self) -> List[TaskSlot]:
        ...
        remaining_minutes = window.available_minutes - int(
            (current_time - window.start).total_seconds() / 60
        )
            ...
            for round_num in range(min(optimization_rounds, 20)):
                for i, task_name in enumerate(optimization_task_names):
                    ...
                    if (
                        ...
                        task_duration = int(
                            (window.end - current_time).total_seconds() / 60
                        )
    ...
    def print_schedule(self, schedule: List[TaskSlot]):
        ...
        print(
            f"🎯 核心任务预算: {self.CORE_TASK_BUDGET_MINUTES}分钟 ({self.CORE_TASK_BUDGET_MINUTES/60:.1f}小时)"
        )
        print("=" * 100)
        ...
        if core_tasks:
            print("\n🔷 核心任务阶段")
            print(
                f"{'#':<3} {'任务':<35} {'开始':<10} {'结束':<10} {'耗时':<8} {'Agent':<15}"
            )
            print("-" * 100)
            ...
            print("-" * 100)
            print(
                f"核心任务总计: {core_actual_total}分钟 ({core_actual_total/60:.1f}小时)"
            )
        ...
        print("=" * 100)
        print(
            f"✅ 核心任务: {len(core_tasks)} 个, {sum(s.duration_minutes for s in core_tasks)}分钟 ({sum(s.duration_minutes for s in core_tasks)/60:.1f}小时)"
        )
        print(
