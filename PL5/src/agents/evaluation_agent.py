        ...
        for pos in positions:
            if pos in test_data.columns:
                ...
                results[pos] = {
                    ...
                    "top_k_accuracy": {
                        "top_3": (
                            sum(sorted(freq_dist.values(), reverse=True)[:3])
                            if len(freq_dist) >= 3
                            else sum(freq_dist.values())
                        ),
                        "top_5": (
                            sum(sorted(freq_dist.values(), reverse=True)[:5])
                            if len(freq_dist) >= 5
                            else sum(freq_dist.values())
                        ),
                        "top_8": (
                            sum(sorted(freq_dist.values(), reverse=True)[:8])
                            if len(freq_dist) >= 8
                            else sum(freq_dist.values())
                        ),
                    },
