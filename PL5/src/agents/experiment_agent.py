...
class ExperimentDesignAgent(BaseAgent):
    ...
    async def _select_by_importance(self, data: pd.DataFrame, target_col: str, n_features: int) -> List[str]:
        ...
        # 准备数据
        feature_cols = [c for c in data.columns if c not in ['period', 'full_number', 'wan', 'qian', 'bai', 'shi', 'ge']]
    ...
    async def _run_experiment(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for feature_set in experiment_config.feature_sets:
            for model_config in experiment_config.model_configs:
                # 准备数据
                X = data[feature_set] if feature_set else data.drop(columns=['period', 'full_number', 'wan', 'qian', 'bai', 'shi', 'ge'])
    ...
    async def _run_cross_validation(self, X: pd.DataFrame, data: pd.DataFrame, 
                                   n_folds: int, model_config: Dict) -> Dict[str, Any]:
        """运行交叉验证"""
    ...
    async def _analyze_results(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        analysis = {
            ...
            'best_experiment': max(experiment_results, 
                                 key=lambda x: x.get('metrics', {}).get('accuracy', 0)),
            'worst_experiment': min(experiment_results, 
                                  key=lambda x: x.get('metrics', {}).get('accuracy', 0))
        }
    ...
    async def _recommend_config(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if 'max_features' in constraints:
            recommended['n_features'] = min(recommended.get('n_features', 100), 
                                           constraints['max_features'])
