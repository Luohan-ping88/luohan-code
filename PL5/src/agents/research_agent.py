...
class ResearchAgent(BaseAgent):
    ...
    async def generate_research_report(self, analysis_result: Dict[str, Any]) -> str:
        ...
        for pos, stats in analysis_result['basic_statistics'].items():
            report_parts.append(f"- {pos}位: 均值={stats['mean']:.2f}, 标准差={stats['std']:.2f}, 范围={stats['min']}-{stats['max']}, 众数={stats['mode']}")
        report_parts.append("")
        ...
        for pos, anomaly in analysis_result['anomaly_detection'].items():
            report_parts.append(f"- {pos}位: 异常数量={anomaly['anomaly_count']}, 异常比例={anomaly['anomaly_ratio']:.2f}")
        report_parts.append("")
        ...
        sorted_correlations = sorted(analysis_result['correlation_analysis'].items(), key=lambda x: abs(x[1]), reverse=True)
