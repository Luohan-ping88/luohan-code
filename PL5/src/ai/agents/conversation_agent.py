...
class ConversationAgent(BaseAgent):
    ...
    def run(self, task: str, context: Optional[Dict[str, Any]] = None) -> ToolResult:
        ...
        # 构建对话消息
        messages = [
            ConversationMessage(role="system", content="你是一个智能对话助手，擅长理解用户需求并提供专业的回答。"),
    ...
    def chat(self, messages: List[ConversationMessage], context: Optional[Dict[str, Any]] = None) -> ToolResult:
        ...
        try:
            ...
            # 构建回复消息
            reply = ConversationMessage(role="assistant", content=response.get("content", ""))

            return ToolResult(
                success=True,
                data={"reply": reply, "conversation": messages + [reply]},
                metadata={"agent_type": self.agent_type.value, "llm_model": self.llm.model_name},
            )
        except Exception as e:
            return ToolResult(
                success=False, error=f"对话执行失败: {str(e)}", metadata={"agent_type": self.agent_type.value}
            )
