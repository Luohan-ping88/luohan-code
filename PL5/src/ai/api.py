...
from .system_health import start_health_monitoring, register_service, HealthCheckResult, ServiceStatus, get_system_status, get_system_metrics, run_diagnostics
...
class RateLimitMiddleware:
    ...
    async def __call__(self, scope, receive, send):
        ...
        # 清理过期记录
        self.requests[client_ip][category] = [
            t for t in self.requests[client_ip][category] 
...
async def run_agent(task: str, context: Optional[Dict[str, Any]] = None, agent_config: Optional[Dict[str, Any]] = None, current_user: Dict[str, Any] = Depends(get_current_user)):
    """运行Agent"""
    try:
        # 构建默认配置
        if not agent_config:
...
def get_memory_items(memory_name: str, limit: int = Query(10, ge=1, le=100), current_user: Dict[str, Any] = Depends(get_current_user)):
    """获取记忆项"""
    memory = memory_manager.get_memory(memory_name)
    if not memory:
        raise HTTPException(status_code=404, detail=f"Memory '{memory_name}' not found")
...
def save_workflow_template(template_name: str, workflow_data: Dict[str, Any], current_user: Dict[str, Any] = Depends(get_current_user)):
    """保存工作流模板"""
    try:
        from .ai_types import WorkflowStep
    ...
    def get_detailed_status(self) -> Dict[str, Any]:
        ...
        if progress >= 100:
            if "采集" in current_task:
                training_steps["data_fetch"] = {"step": "数据采集", "status": "completed", "time": status.get("last_run", ""), "desc": "已完成"}
            if "特征" in current_task or "采集" in current_task:
                training_steps["feature_engineering"] = {"step": "特征提取", "status": "completed", "time": status.get("last_run", ""), "desc": "已完成"}
            if "训练" in current_task or "特征" in current_task or "采集" in current_task:
                training_steps["training"] = {"step": "模型训练", "status": "completed", "time": status.get("last_run", ""), "desc": "已完成"}
            training_steps["prediction"] = {"step": "生成预测", "status": "completed", "time": status.get("last_run", ""), "desc": "已完成"}
        elif progress > 0:
            if "采集" in current_task:
                training_steps["data_fetch"] = {"step": "数据采集", "status": "completed", "time": status.get("last_run", ""), "desc": "已完成"}
                training_steps["feature_engineering"] = {"step": "特征提取", "status": "running", "time": "进行中", "desc": f"进行中 ({progress}%)"}
                ...
                training_steps["data_fetch"] = {"step": "数据采集", "status": "completed", "time": status.get("last_run", ""), "desc": "已完成"}
                training_steps["feature_engineering"] = {"step": "特征提取", "status": "completed", "time": status.get("last_run", ""), "desc": "已完成"}
                training_steps["training"] = {"step": "模型训练", "status": "running", "time": "进行中", "desc": f"进行中 ({progress}%)"}
            ...
            try:
                ...
                for line in reversed(lines[-2000:]):
                    ...
                    if "数据采集" in line and "完成" in line:
                        training_steps["data_fetch"] = {"step": "数据采集", "status": "completed", "time": self._extract_time(line), "desc": "最新数据已入库"}
                        break
                    elif "特征" in line and "完成" in line:
                        training_steps["feature_engineering"] = {"step": "特征提取", "status": "completed", "time": self._extract_time(line), "desc": "特征工程完成"}
                        break
                    elif "训练" in line and "完成" in line:
                        training_steps["training"] = {"step": "模型训练", "status": "completed", "time": self._extract_time(line), "desc": "模型训练完成"}
                        break
                    elif "预测" in line and ("完成" in line or "生成" in line):
                        training_steps["prediction"] = {"step": "生成预测", "status": "completed", "time": self._extract_time(line), "desc": "预测已生成"}
                        break
            except:
                pass
...
async def run_security_scan(input_data: Optional[Any] = None, config: Optional[Dict[str, Any]] = None, directory: str = ".", current_user: Dict[str, Any] = Depends(get_current_user)):
    """运行安全扫描"""
    if current_user["role"] != "admin":
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
...
async def register_service(service_id: str, service_url: str, weight: int = 1, current_user: Dict[str, Any] = Depends(get_current_user)):
    """注册服务到负载均衡器"""
    if current_user["role"] != "admin":
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
...
async def scale_auto_scaler(cpu_usage: float, memory_usage: float, current_user: Dict[str, Any] = Depends(get_current_user)):
    """执行自动扩展决策"""
    if current_user["role"] != "admin":
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
    ...
    try:
        ...
        if scheduler_config_file.exists():
            with open(scheduler_config_file, 'r', encoding='utf-8') as f:
                ...
                daily_cycle_tasks = [
                    ...
                    {"time": config.get("evaluation_time", "22:15"), "name": "评估预测", "desc": "评估预测逻辑与命中情况"},
                    {"time": config.get("optimization_start", "22:45"), "name": "策略优化", "desc": "推理逻辑策略优化学习"},
                    {"time": config.get("training_start", "00:30"), "name": "深度训练", "desc": "开始深度训练模型"},
                    {"time": config.get("incremental_training_morning", "08:00"), "name": "增量训练(上午)", "desc": "首次佐证前增量学习"},
                    {"time": config.get("first_prediction_verification", "10:00"), "name": "佐证1", "desc": "首次预测验证"},
                    {"time": config.get("incremental_training_noon", "12:00"), "name": "增量训练(中午)", "desc": "二次佐证前增量学习"},
                    {"time": config.get("second_prediction_verification", "13:00"), "name": "佐证2", "desc": "二次预测验证"},
                    {"time": config.get("incremental_training_afternoon", "14:00"), "name": "增量训练(下午)", "desc": "三次佐证前增量学习"},
                    {"time": config.get("third_prediction_verification", "15:00"), "name": "佐证3", "desc": "三次预测验证"},
                    {"time": config.get("deep_strategy_optimization", "16:00"), "name": "深度策略优化", "desc": "深度策略优化（四次佐证）"},
                    {"time": config.get("prediction_preview", "17:00"), "name": "预测预生成", "desc": "预测结果预生成（五次佐证）"},
                    {"time": config.get("final_prediction_time", "18:00"), "name": "最终预测", "desc": "生成最终预测结果"},
                    {"time": config.get("final_prediction_verification_time", "19:00"), "name": "佐证6", "desc": "验证最终预测结果"},
                    {"time": config.get("pre_sale_prediction_time", "20:00"), "name": "售前预测", "desc": "售前最终预测"},
                    {"time": config.get("email_send_time", "20:15"), "name": "发送报告", "desc": "发送训练报告和预测到邮箱"}
                ]
        ...
        return {
            ...
            "daily_cycle": {
                ...
                "data_fetch_time": config.get("data_fetch_time", "22:15") if scheduler_config_file.exists() else "22:15",
                "training_deadline": config.get("training_deadline", "17:00") if scheduler_config_file.exists() else "17:00",
                "final_prediction_time": config.get("final_prediction_time", "18:00") if scheduler_config_file.exists() else "18:00"
            },
        ...
        return {
            "latest_period": "未知",
            "next_period": "未知",
            "daily_cycle": {"enabled": True, "data_fetch_time": "22:15", "training_deadline": "17:00", "final_prediction_time": "18:00"},
            "training_status": [],
...
async def get_pl5_daily_cycle_timeline(current_user: Dict[str, Any] = Depends(get_current_user)):
    ...
    try:
        ...
        timeline = [
            {"id": 1, "time": config.get("data_fetch_time", "22:15"), "name": "数据采集", "type": "fetch", "desc": "自动获取开奖数据"},
            {"id": 2, "time": config.get("evaluation_time", "22:15"), "name": "评估预测", "type": "evaluate", "desc": "评估预测逻辑与命中情况"},
            {"id": 3, "time": config.get("optimization_start", "22:45"), "name": "策略优化", "type": "optimize", "desc": "推理逻辑策略优化学习"},
            {"id": 4, "time": config.get("training_start", "00:30"), "name": "深度训练", "type": "train", "desc": "开始深度训练模型", "critical": True},
            {"id": 5, "time": config.get("incremental_training_morning", "08:00"), "name": "增量训练(上午)", "type": "incremental", "desc": "首次佐证前增量学习"},
            {"id": 6, "time": config.get("first_prediction_verification", "10:00"), "name": "佐证1", "type": "verify", "desc": "首次预测验证"},
            {"id": 7, "time": config.get("incremental_training_noon", "12:00"), "name": "增量训练(中午)", "type": "incremental", "desc": "二次佐证前增量学习"},
            {"id": 8, "time": config.get("second_prediction_verification", "13:00"), "name": "佐证2", "type": "verify", "desc": "二次预测验证"},
            {"id": 9, "time": config.get("incremental_training_afternoon", "14:00"), "name": "增量训练(下午)", "type": "incremental", "desc": "三次佐证前增量学习"},
            {"id": 10, "time": config.get("third_prediction_verification", "15:00"), "name": "佐证3", "type": "verify", "desc": "三次预测验证"},
            {"id": 11, "time": config.get("deep_strategy_optimization", "16:00"), "name": "深度策略优化", "type": "optimize", "desc": "深度策略优化（四次佐证）"},
            {"id": 12, "time": config.get("prediction_preview", "17:00"), "name": "预测预生成", "type": "predict", "desc": "预测结果预生成（五次佐证）"},
            {"id": 13, "time": config.get("final_prediction_time", "18:00"), "name": "最终预测", "type": "predict", "desc": "生成最终预测结果", "critical": True},
            {"id": 14, "time": config.get("final_prediction_verification_time", "19:00"), "name": "佐证6", "type": "verify", "desc": "验证最终预测结果"},
            {"id": 15, "time": config.get("pre_sale_prediction_time", "20:00"), "name": "售前预测", "type": "predict", "desc": "售前最终预测", "critical": True},
            {"id": 16, "time": config.get("email_send_time", "20:15"), "name": "发送报告", "type": "report", "desc": "发送训练报告和预测到邮箱"}
        ]
