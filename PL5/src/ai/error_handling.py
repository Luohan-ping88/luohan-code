...
class AIError(Exception):
    ...
    def __init__(self, message: str, error_type: ErrorType = ErrorType.UNKNOWN_ERROR, 
                 error_code: Optional[int] = None, details: Optional[dict] = None):
        """初始化错误
...
class RetryResult(Generic[T]):
    ...
    def __init__(self, success: bool, result: Optional[T] = None, 
                 error: Optional[AIError] = None, attempts: int = 1):
        """初始化重试结果
...
def retry_with_backoff(func: Callable[..., T], config: Optional[RetryConfig] = None, 
                       *args, **kwargs) -> RetryResult[T]:
    """带退避的重试装饰器
    ...
    while attempts <= config.max_retries:
        try:
            ...
            if attempts <= config.max_retries:
                ...
                logger.info(f"Retrying {func.__name__} in {delay:.2f} seconds... (Attempt {attempts}/{config.max_retries})")
                time.sleep(delay)
