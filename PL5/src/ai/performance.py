...
class LoadBalancer:
    ...
    def get_service(self) -> Optional[str]:
        ...
        # 过滤健康的服务
        healthy_services = [
            (service_id, info) for service_id, info in self.services.items()
            if info["health"]
        ]
...
class AutoScaler:
    ...
    def __init__(self, min_instances: int = 1, max_instances: int = 10, 
                 cpu_threshold: float = 0.7, memory_threshold: float = 0.8):
        """初始化自动扩展器
    ...
    def scale(self, cpu_usage: float, memory_usage: float) -> Dict[str, Any]:
        ...
        if cpu_usage > self.cpu_threshold or memory_usage > self.memory_threshold:
            ...
            if current_instances > self.min_instances:
                ...
                decision = {"action": "scale_down", "instances": current_instances - 1, "instance_id": removed_instance_id}
