    ...
    if precomputed_predictions is not None:
        ...
        if model_loaded and hasattr(predictor, "feature_cols") and predictor.feature_cols:
            ...
            if old_feature_count != new_feature_count:
                logger.warning(
                    f"  特征维度不匹配: 旧模型{old_feature_count}维，新数据{new_feature_count}维，需要重新训练"
                )
                need_retrain = True

        v10_complete = (
            model_loaded
            and getattr(predictor, "mamba_predictor", None) is not None
            and getattr(predictor, "itransformer_predictor", None) is not None
    ...
    if hasattr(predictor, "hmm_models"):
        for pos in positions:
            if pos in predictor.hmm_models:
                ...
                state = "unknown"
                state_prob = 0.0
            hmm_states[pos] = {"state": state, "probability": state_prob}
