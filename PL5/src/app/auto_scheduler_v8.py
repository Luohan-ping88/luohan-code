...
class AutoSchedulerV8:
    ...
    def task_optimize(self):
        ...
        try:
            ...
            evaluation_result = evaluator.evaluate_all_strategies(
                test_window=30, target_duration_minutes=target_duration_minutes
            )
                        ...
                        if last_updated_str:
                            try:
                                ...
                                if cache_age > timedelta(hours=24):
                                    logger.info(
                                        f"[_get_best_feature_config] 缓存已过期（{cache_age.total_seconds()/3600:.1f}小时），将执行动态验证"
                                    )
                                    force_validate = True
                                    break
                            except Exception as e:
        ...
        try:
            ...
            if (
                ...
                if old_feature_count != new_feature_count:
                    logger.warning(
                        f"  特征维度不匹配: 旧模型{old_feature_count}维，新数据{new_feature_count}维，强制重新训练"
                    )
                    should_retrain = True
    ...
    def setup_schedule(self):
        ...
        schedule.every().day.at(prediction_preview).do(
            lambda: self._schedule_thread_wrapper(self.task_prediction_preview)
        )
        logger.info(f"[OK] {prediction_preview} - 预测结果预生成（五次佐证）")

        # 任务11: 生成最终预测结果 (18:00)
        schedule.every().day.at(final_prediction_time).do(
            lambda: self._schedule_thread_wrapper(self.task_final_prediction)
        )
    ...
    def check_intelligent_scheduling(self):
        ...
        if hasattr(self, "start_time"):
            ...
            if elapsed < 300:  # 启动后5分钟内
                logger.info(
                    f"[智能调度] 调度器刚启动 {elapsed:.0f} 秒，等待 schedule 定时器触发（还需等待 {300 - elapsed:.0f} 秒）"
                )
                return
        ...
        if now >= today_2200:
            ...
            if (
                ...
                logger.debug(
                    f"[智能调度] 不在日循环周期内，距下一个日循环还有 {(today_2200 - now).seconds // 60} 分钟"
                )  # 【V10.4修复】降为DEBUG
            return
        ...
        if saved_cycle_date and saved_cycle_date != current_cycle_date:
            logger.info(
                f"[智能调度] 检测到新周期（当前: {current_cycle_date}, 保存: {saved_cycle_date}），重置工作流状态"
            )
