...
class PL5AutomationScheduler:
    ...
    def start(self):
        ...
        if not self.is_running:
            ...
            self.data_collection_job = self.scheduler.enter(
                time_until_midnight,
                1,
                self._run_data_collection_wrapper
            )
            ...
            self.system_check_job = self.scheduler.enter(
                0,
                2,
                self._check_system_status_wrapper
            )
            ...
            self.performance_check_job = self.scheduler.enter(
                0,
                3,
                self._check_performance_wrapper
            )
    ...
    def _check_performance(self):
        ...
        try:
            ...
            logger.info(f"[Automation] 性能瓶颈检测完成: 系统瓶颈 {system_bottlenecks}, 函数瓶颈 {function_bottlenecks}, 性能趋势 {trends}")
    ...
    async def _run_training(self):
        ...
        try:
            ...
            if training_result.get('success'):
                ...
                handle_failure('training_failure', Exception(training_result.get('error', '训练失败')), {
                    'orchestrator': self.orchestrator
                })
        except Exception as e:
            logger.error(f"[Automation] 训练任务执行失败: {str(e)}")
            handle_failure('training_failure', e, {
                'orchestrator': self.orchestrator
            })
    ...
    async def _run_report_generation(self):
        ...
        try:
            ...
            if prediction_result.get('success'):
                ...
                handle_failure('prediction_failure', Exception(prediction_result.get('error', '预测失败')), {
                    'orchestrator': self.orchestrator
                })
        except Exception as e:
            logger.error(f"[Automation] 报告生成和发送任务执行失败: {str(e)}")
            handle_failure('report_generation_failure', e, {
                'orchestrator': self.orchestrator
            })
