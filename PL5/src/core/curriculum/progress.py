...
class ProgressTracker:
    ...
    def _update_skill_mastery(self, skill_id: str, is_correct: bool) -> None:
        ...
        if is_correct:
            skill.mastery_level = min(
                skill.mastery_level + learning_rate * (1 - skill.mastery_level),
                1.0
            )
        else:
            skill.mastery_level = max(
                skill.mastery_level - learning_rate * skill.mastery_level,
                0.0
            )
    ...
    def get_overall_stats(self) -> Dict[str, any]:
        ...
        skill_stats = {
            ...
            'average_mastery': (
                sum(s.mastery_level for s in self.skills.values()) / len(self.skills)
        ...
        if self.storage_path.exists():
            try:
                ...
                for session_data in data.get('sessions', []):
                    ...
                    session.records = [
                        self.records[r['record_id']] for r in session_records
