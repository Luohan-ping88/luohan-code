...
class TestingManager:
    ...
    def _select_questions(
        ...
        for skill_id in skill_ids:
            skill_questions = [
                q for q in self.question_bank.get(skill_id, [])
    ...
    def submit_test(
        ...
        time_spent = (
            datetime.now() - test.start_time
        ).total_seconds() if test.start_time else 0.0
    ...
    def _update_progress_from_test(self, test: Test, result: TestResult) -> None:
        ...
        for skill_id, performance in result.skill_performance.items():
            ...
            self.progress_tracker.record_learning(
                ...
                difficulty_score=0.5 if test.difficulty == DifficultyLevel.MEDIUM else (
