...
class DataAugmenter:
    ...
    def smote_augmentation(self, X: np.ndarray, y: np.ndarray, sampling_strategy: str = 'auto', k_neighbors: int = 5) -> Tuple[np.ndarray, np.ndarray]:
        """使用SMOTE技术进行数据增强"""
        try:
            # 确保y是一维数组
            if len(y.shape) > 1:
    ...
    def random_oversampling(self, X: np.ndarray, y: np.ndarray, sampling_strategy: Dict = None) -> Tuple[np.ndarray, np.ndarray]:
        """随机过采样"""
    ...
    def feature_perturbation(self, X: np.ndarray, y: np.ndarray, perturbation_rate: float = 0.1, perturbation_std: float = 0.05) -> Tuple[np.ndarray, np.ndarray]:
        """特征扰动增强"""
    ...
    def augment_data(self, df: pd.DataFrame, feature_cols: List[str], target_cols: List[str], augmentation_config: Dict = None) -> Tuple[np.ndarray, np.ndarray]:
        """综合数据增强"""
