...
class AdvancedDataValidator:
        ...
        for idx, row in data.iterrows():
            ...
            if is_valid:
                ...
                for issue in row_issues:
                    self._add_issue(issue['type'], f"记录 {idx}: {issue['message']}", 
                                   record_index=idx, severity=issue.get('severity', 'warning'))
    ...
    def _validate_structure(self, data: pd.DataFrame):
        ...
        if missing_columns:
            self._add_issue(DataIssue.MISSING_VALUES, f"缺少必需列: {missing_columns}", 
                           severity="critical")
        ...
        for col, rule in self.rules.items():
            if col in data.columns:
                ...
                if expected_type == int and not pd.api.types.is_integer_dtype(actual_type):
                    self._add_issue(DataIssue.INVALID_FORMAT, f"列 {col} 类型错误: 期望 {expected_type}, 实际 {actual_type}")
    ...
    def _validate_dataset_level(self, data: pd.DataFrame):
        ...
        if duplicates.any():
            ...
            self._add_issue(DataIssue.DUPLICATES, f"发现重复期号: {duplicate_periods[:5]}", 
        ...
        for pos in positions:
            if pos in data.columns:
                ...
                if len(invalid_values) > 0:
                    self._add_issue(DataIssue.OUT_OF_RANGE, 
                ...
                if len(values) > 100:
                    ...
                    if chi_square > 20:  # 卡方检验阈值
                        self._add_issue(DataIssue.ANOMALY, 
    ...
    def _validate_missing_periods(self, data: pd.DataFrame):
        ...
        try:
            ...
            if missing_periods:
                self._add_issue(DataIssue.SEQUENCE_BREAK, 
    ...
    def _create_result(self, is_valid: bool, data: pd.DataFrame, data_hash: str = None) -> ValidationResult:
        ...
        summary = {
            ...
            'validity_rate': self.validation_stats.get('valid_records', 0) / max(self.validation_stats.get('total_records', 1), 1),
...
def validate_data_file(data_path: Path, validation_level: ValidationLevel = ValidationLevel.STANDARD) -> ValidationResult:
    """验证数据文件"""
