...
class FeatureEngineerV9:
    ...
    def __init__(
        ...
        self.enable_parallel = (
            enable_parallel and parallel_cfg.get("enable", True)
            if parallel_cfg
            else (enable_parallel and JOBLIB_AVAILABLE)
        )
    ...
    def extract_all_features(
        ...
        duration = time.time() - start_time
        logger.info(
            f"V10.0 特征工程完成: {result_df.shape[1]} 列, 耗时: {duration:.2f}s | 缓存统计: {self.cache.stats}"
        )
        return result_df
    ...
    def _select_features(
        ...
        logger.info(
            f"智能特征选择: 数据量={n_samples}, 总特征={n_total_features}, "
