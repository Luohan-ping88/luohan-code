...
class UncertaintyDecomposer:
    ...
    def decompose_ensemble(predictions: List[np.ndarray]) -> Dict:
        ...
        epistemic = float(np.mean(
            np.sum(predictions * np.log(predictions + 1e-10) - 
                   predictions * np.log(mean_pred + 1e-10), axis=-1)
        ))
...
class DistributionShiftDetector:
    ...
    @staticmethod
    def mmd(x: np.ndarray, y: np.ndarray, kernel: str = 'rbf',
...
class EnhancedBayesianQuantifier:
    ...
    def __init__(self, calibration_alpha: float = 0.1,
    ...
    def fit(self, model_predictions: Dict[str, List[np.ndarray]],
    ...
    def quantify(self, model_predictions: Dict[str, np.ndarray],
                 training_stats: Optional[Dict] = None) -> Dict:
        """量化预测的不确定性
    ...
    def get_confidence_adjusted_weights(self, model_predictions: Dict[str, np.ndarray],
