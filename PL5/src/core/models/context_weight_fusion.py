...
class ContextAwareWeightFusion:
    ...
    MODEL_NAMES: List[str] = ['stacking', 'hmm', 'copula', 'bayesian', 'mamba']
    ...
    def __init__(
        ...
        self.enable_online_update: bool = enable_online_update
        ...
        self.weight_adjustment_factor: float = 0.1
        
        self._init_weight_predictor()
    
    def _init_weight_predictor(self) -> None:
        """初始化简化的权重预测器 (无外部依赖的纯NumPy实现)"""
        np.random.seed(42)
        
        self.context_encoder_weights: np.ndarray = np.random.randn(32, self.context_dim) * 0.1
        self.context_encoder_bias: np.ndarray = np.zeros(self.context_dim)
        
        self.weight_predictor_weights: np.ndarray = np.random.randn(self.context_dim, len(self.MODEL_NAMES)) * 0.1
        self.weight_predictor_bias: np.ndarray = np.zeros(len(self.MODEL_NAMES))
    ...
    def _extract_context_features(self) -> np.ndarray:
        ...
        [27:32) - 趋势特征 (5维)
        """
        features: np.ndarray = np.zeros(self.context_dim)
        idx: int = 0
    ...
    def _predict_weights(self, context: np.ndarray) -> np.ndarray:
        ...
        基于上下文预测权重
        """
        logits: np.ndarray = context @ self.weight_predictor_weights + self.weight_predictor_bias
        
        exp_logits: np.ndarray = np.exp(logits - logits.max())
        weights: np.ndarray = exp_logits / exp_logits.sum()
    ...
    def get_weights(
        ...
        Returns:
            {模型名: 权重}
        """
        raw_features: np.ndarray = self._extract_context_features()
        context: np.ndarray = self._encode_context(raw_features)
        raw_weights: np.ndarray = self._predict_weights(context)
        ...
        self.current_weights: Dict[str, float] = dict(zip(self.MODEL_NAMES, raw_weights))
        self.current_context: np.ndarray = context
    ...
    def fuse_predictions(
        ...
        Returns:
            融合后的概率分布
        """
        weights: Dict[str, float] = self.get_weights()
    ...
    def _confidence_weighted_fusion(
        ...
        for model_name, proba in predictions.items():
            ...
            confidence: float = (max_prob - entropy / np.log(10)) / 2
            
            confidence = np.clip(confidence, 0, 1)
            adjusted_weight: float = base_w * (1 + confidence * self.confidence_temperature)
    ...
    def update_with_feedback(
        ...
        for model_name, pos_predictions in predictions.items():
            ...
            for pos in self.POSITIONS:
                ...
                pred_list: List[int] = pos_predictions[pos][:top_k]
                actual_val: int = actual[pos]
                
                hit: bool = actual_val in pred_list
                rank: int = pred_list.index(actual_val) + 1 if hit else 0
                
                reward: float = 1.0 if hit else 0.0
                if hit:
                    reward += 1.0 / rank
            ...
            if count > 0:
                avg_reward: float = total_reward / count
    ...
    def _update_weight_predictor(self) -> None:
        """
        基于历史表现更新权重预测器
        ...
        rewards: np.ndarray = np.array(list(self.reward_history))
        
        if len(rewards) < 10:
            return
        
        half: int = len(rewards) // 2
        first_half_reward: float = np.mean(rewards[:half])
        second_half_reward: float = np.mean(rewards[half:])
        
        improvement: float = second_half_reward - first_half_reward
        
        adjustment: float = improvement * 0.01
        
        for i in range(len(self.MODEL_NAMES)):
            if self.model_performance[self.MODEL_NAMES[i]]:
                recent = list(self.model_performance[self.MODEL_NAMES[i]])
                avg_reward: float = np.mean([r.get('reward', 0) for r in recent])
...
class ThompsonSamplingOptimizer:
    ...
    def __init__(
        ...
        self.prior: Dict[str, float] = {'alpha': initial_alpha, 'beta': initial_beta}
    ...
    def update(self, hit_results: Dict[str, bool]) -> None:
        """
        根据预测结果更新后验分布
    ...
    def sample_weights(self, n_samples: int = 1000) -> Dict[str, float]:
        ...
        Returns:
            {模型名: 采样权重}
        """
        samples: np.ndarray = np.zeros((n_samples, len(self.model_names)))
        
        for i, model in enumerate(self.model_names):
            alpha: float = self.posterior[model]['alpha']
            beta: float = self.posterior[model]['beta']
            samples[:, i] = np.random.beta(alpha, beta, n_samples)
        
        normalized_samples: np.ndarray = samples / samples.sum(axis=1, keepdims=True)
        mean_weights: np.ndarray = normalized_samples.mean(axis=0)
    ...
    def get_confidence_interval(
        ...
        lower: float = alpha / (alpha + beta)
        
        z: float = 1.96 if confidence == 0.95 else 2.576
        margin: float = z * np.sqrt(alpha * beta / ((alpha + beta)**2 * (alpha + beta + 1)))
