...
class BottleneckDetector:
        ...
        for func_name, stats in all_stats.items():
            ...
            if stats['avg_execution_time'] > self.thresholds['execution_time']:
                bottlenecks.append({
                    ...
                    'severity': 'high' if stats['avg_execution_time'] > self.thresholds['execution_time'] * 2 else 'medium',
    ...
    def save_bottleneck_report(self, report_path: Optional[Path] = None):
        ...
        if report_path is None:
            report_path = Path("logs") / "performance" / f"bottleneck_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
