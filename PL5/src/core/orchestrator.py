...
class PL5Orchestrator:
    ...
    async def _stage_feature_engineering(self, prev_result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            feature_cols = [c for c in df_features.columns if c not in ['period', 'date', 'full_number', 'parse_line', 'wan', 'qian', 'bai', 'shi', 'ge']]
    ...
    async def _stage_model_evaluation(self, train_result: Dict[str, Any], 
        ...
        try:
            ...
            self_learning.record_evaluation(accuracy, {
        ...
        try:
            ...
            report = {
                ...
                'analysis': {
                    ...
                    'performance_metrics': {
                        'overall_accuracy': all_results.get('model_evaluation', {}).get('evaluation', {}).get('overall_accuracy', 0.0),
                        'total_predictions': all_results.get('model_evaluation', {}).get('evaluation', {}).get('total_predictions', 0),
                        'correct_predictions': all_results.get('model_evaluation', {}).get('evaluation', {}).get('correct_predictions', 0)
                    },
                    'feature_analysis': {
            ...
            try:
                ...
                if best_config_path.exists():
                    try:
                        ...
                        if best_select_top is not None:
                            logger.info(f"[_stage_report_generation] 使用动态验证最佳配置: select_top={best_select_top}")
                    except Exception:
                        pass
                ...
                all_feature_cols = [c for c in df_features.columns if c not in ['period', 'date', 'full_number', 'parse_line', 'wan', 'qian', 'bai', 'shi', 'ge']]
                ...
                if load_result:
                    ...
                    if predictor.feature_cols and len(predictor.feature_cols) > 0:
                        ...
                        if missing:
                            logger.warning(f"[_stage_report_generation] 模型特征列中有 {len(missing)} 个缺失，将用0填充")
                            for col in missing:
                                df_features[col] = 0.0
                    ...
                    # 生成8个预测号码
                    predictions_8 = predictor.predict(latest_features, recent_original_data=recent_original_data, top_k=8)
                    # 生成5个预测号码
                    predictions_5 = predictor.predict(latest_features, recent_original_data=recent_original_data, top_k=5)
                    # 生成3个预测号码
                    predictions_3 = predictor.predict(latest_features, recent_original_data=recent_original_data, top_k=3)
        ...
        try:
            ...
            for config_dir in [LOGS_DIR, MODELS_DIR]:
                ...
                if best_config_path.exists():
                    try:
                        ...
                        logger.warning(f"[Orchestrator] 读取 {config_dir.name}/best_feature_config.json 失败: {cfg_err}")
            ...
            if cfg_data is not None:
                try:
                    ...
                    if best_select_top is not None:
                        logger.info(f"[Orchestrator] 读取到动态验证最佳配置: select_top={best_select_top}，将用于生产预测")
                    else:
                        logger.info("[Orchestrator] 动态验证最佳配置中 select_top=None（已验证全量特征最优），使用全量特征")
                except Exception as cfg_err:
                    logger.warning(f"[Orchestrator] 解析 best_feature_config.json 失败（非致命）: {cfg_err}，使用默认行为")
