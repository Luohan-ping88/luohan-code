...
class PL5OrchestratorOptimized:
    ...
    def __init__(
        ...
        logger.info(
            f"[OrchestratorOptimized] 初始化完成，workflow_dir={self._workflow_dir}, timeout={self._default_timeout}s"
        )
    ...
    async def execute_training_pipeline(
        self, params: Dict[str, Any] = None, timeout: Optional[int] = None
    ) -> Dict[str, Any]:
        """
        执行训练流程（带超时控制）
    ...
    async def _stage_model_evaluation(
        ...
        try:
            ...
            self.self_learning.record_evaluation(
                accuracy,
                {
