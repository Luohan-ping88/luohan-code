...
class TestDataToFeaturesIntegration:
    ...
    def test_data_validation_to_feature_engineering(self, sample_data):
        ...
        # 检查特征列
        feature_cols = [
            col for col in features.columns if col not in ["period", "full_number", "wan", "qian", "bai", "shi", "ge"]
        ]
...
class TestModelTrainingIntegration:
    ...
    def test_feature_to_model_training(self, sample_features):
        ...
        # 1. 准备特征和目标
        feature_cols = [
            col for col in sample_features.columns if col not in ["period", "wan", "qian", "bai", "shi", "ge"]
        ]
    ...
    def test_model_prediction_integration(self, sample_features):
        ...
        recent_original_data = {
            pos: sample_features[pos].values[-10:] for pos in ["wan", "qian", "bai", "shi", "ge"]  # 最近10条数据
        }

        # 生成预测
        predictions = predictor.predict(latest_features, recent_original_data=recent_original_data, top_k=3)
...
class TestSystemFlow:
    ...
    def test_complete_system_flow(self, tmp_path):
        ...
        try:
            ...
            recent_original_data = {pos: data[pos].values[-5:] for pos in ["wan", "qian", "bai", "shi", "ge"]}

            predictions = predictor.predict(latest_features, recent_original_data=recent_original_data, top_k=2)
