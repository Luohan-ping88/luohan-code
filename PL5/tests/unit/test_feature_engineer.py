...
class TestFeatureImportanceAnalyzer:
    ...
    def test_calculate_importance(self, analyzer, sample_data):
        ...
        importance = analyzer.calculate_importance(
            sample_data, y, method='random_forest'
        )
...
class TestFeatureEngineerV9:
    ...
    def test_extract_features_with_cache(self, engineer, sample_pl5_data):
        """测试特征提取缓存"""
        # 第一次提取
        result1 = engineer.extract_all_features(
            sample_pl5_data,
            select_top=None,
            enable_scaler=False
        )
        ...
        # 第二次提取（应该命中缓存）
        result2 = engineer.extract_all_features(
            sample_pl5_data,
            select_top=None,
            enable_scaler=False
        )
